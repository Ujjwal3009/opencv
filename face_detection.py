...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text):
    ...
    for (x, y, w, h) in features:
        ...
        cv2.putText(img, text, (x, y-6), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
        coords = [x, y, w, h]
    return coords
...
def detect(img, faceCascade, eyeCascade ,noseCascade):
    color = {"vivek":(0,0,100), "red":(0,0,255), "green":(0,255,0), "white":(255,255,255)}
    coords = draw_boundary(img, faceCascade, 1.1, 10, color['vivek'], "Face")
    # If feature is detected, the draw_boundary method will return the x,y coordinates and width and height of rectangle else the length of coords will be 0
    if len(coords) == 4:
        roi_img = img[coords[1]:coords[1]+coords[3], coords[0]:coords[0]+coords[2]]
        coords = draw_boundary(roi_img, eyeCascade, 1.1, 14, color['red'], "eye")
        coords = draw_boundary(roi_img, noseCascade, 1.1, 4, color['white'], "nose")
